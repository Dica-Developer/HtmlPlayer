...
(function(ns) {
  ...
  ID3v1.readTagsFromData = function(data) {
    ...
    var header = data.substr(offset, 3);
    if (header == "TAG") {
      var title = data.substr(offset + 3, 30).replace(/\0/g, "");
      var artist = data.substr(offset + 33, 30).replace(/\0/g, "");
      var album = data.substr(offset + 63, 30).replace(/\0/g, "");
      var year = data.substr(offset + 93, 4).replace(/\0/g, "");

      var trackFlag = ID3.getByteAt(data, offset + 97 + 28);
      if (trackFlag == 0) {
        var comment = data.substr(offset + 97, 28).replace(/\0/g, "");
        var track = ID3.getByteAt(data, offset + 97 + 29);
      } else {
        var comment = "";
        var track = 0;
      }

      var genreIdx = ID3.getByteAt(data, offset + 97 + 30);
      if (genreIdx < 255) {
