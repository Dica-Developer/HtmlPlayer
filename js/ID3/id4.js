...
(function(ns) {
  ...
  ID4.loadData = function(data, callback) {
    ...
    data.loadRange([0, 7], function() {
      loadAtom(data, 0, data.length, callback);
    });
  };
  ...
  function loadAtom(data, offset, length, callback) {
    ...
    var atomSize = ID3.getLongAt(data, offset, true);
    if (atomSize == 0)
      return callback();
    var atomName = data.substr(offset + 4, 4);
  ...
  ID4.readTagsFromData = function(data) {
    var tag = {};
    readAtom(tag, data, 0, data.length);
    return tag;
  };
  ...
  function readAtom(tag, data, offset, length, indent) {
    ...
    while (seek < offset + length) {
      var atomSize = ID3.getLongAt(data, seek, true);
      if (atomSize == 0)
        return;
      var atomName = data.substr(seek + 4, 4);
      // Container atoms
      if (['moov', 'udta', 'meta', 'ilst'].indexOf(atomName) > -1) {
        if (atomName == 'meta')
          seek += 4;
      ...
      if (ID4.atom[atomName]) {
        var klass = ID3.getInteger24At(data, seek + 16 + 1, true);
        var atom = ID4.atom[atomName];
        var type = ID4.types[klass];
        if (atomName == 'trkn') {
          tag[atom[0]] = ID3.getByteAt(data, seek + 16 + 11);
          tag['count'] = ID3.getByteAt(data, seek + 16 + 13);
        } else {
          ...
          switch( type ) {
            case 'text':
              tag[atom[0]] = ID3.getStringWithCharsetAt(data, dataStart, dataEnd, "UTF-8");
              break;

            case 'uint8':
              tag[atom[0]] = ID3.getShortAt(data, dataStart);
              break;
            ...
            case 'png':
              tag[atom[0]] = {
                format : "image/" + type,
                data : ID3.getBytesAt(data, dataStart, dataEnd)
              };
              break;
          }
        }
