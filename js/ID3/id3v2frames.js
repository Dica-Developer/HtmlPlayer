 ...
 */
(function() {
  var pictureType = ["32x32 pixels 'file icon' (PNG only)", "Other file icon", "Cover (front)", "Cover (back)", "Leaflet page", "Media (e.g. lable side of CD)", "Lead artist/lead performer/soloist", "Artist/performer", "Conductor", "Band/Orchestra", "Composer", "Lyricist/text writer", "Recording Location", "During recording", "During performance", "Movie/video screen capture", "A bright coloured fish", "Illustration", "Band/artist logotype", "Publisher/Studio logotype"];
  ...
  function getTime(duration) {
    var duration = duration / 1000, seconds = Math.floor(duration) % 60, minutes = Math.floor(duration / 60) % 60, hours = Math.floor(duration / 3600);
  ...
  ID3v2.readFrameData['APIC'] = function readPictureFrame(offset, length, data, flags, v) {
    ...
    var start = offset;
    var charset = getTextEncoding(ID3.getByteAt(data, offset));
    switch( v ) {
      case '2':
        var format = data.substr(offset + 1, 3);
        offset += 4;
        break;

      case '3':
      case '4':
        var format = ID3.getStringWithCharsetAt(data, offset + 1, length - (offset - start), charset);
        offset += 1 + format.bytesReadCount;
        break;
    }
    var bite = data.getByteAt(offset, 1);
    var type = pictureType[bite];
    var desc = ID3.getStringWithCharsetAt(data, offset + 1, length - (offset - start), charset);
  ...
  ID3v2.readFrameData['COMM'] = function readCommentsFrame(offset, length, data) {
    var start = offset;
    var charset = getTextEncoding(ID3.getByteAt(data, offset));
    var language = data.substr(offset + 1, 3);
    var shortdesc = ID3.getStringWithCharsetAt(data, offset + 4, length - 4, charset);

    offset += 4 + shortdesc.bytesReadCount;
    var text = ID3.getStringWithCharsetAt(data, offset, (start + length) - offset, charset);
  ...
  ID3v2.readFrameData['T*'] = function readTextFrame(offset, length, data) {
    ...
    return ID3.getStringWithCharsetAt(data, offset + 1, length - 1, charset).toString();
  };
  ...
  ID3v2.readFrameData['USLT'] = function readLyricsFrame(offset, length, data) {
    ...
    var charset = getTextEncoding(ID3.getByteAt(data, offset));
    var language = data.substr(offset + 1, 3);
    var descriptor = ID3.getStringWithCharsetAt(data, offset + 4, length - 4, charset);

    offset += 4 + descriptor.bytesReadCount;
    var lyrics = ID3.getStringWithCharsetAt(data, offset, (start + length) - offset, charset);
