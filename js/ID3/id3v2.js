...
(function(ns) {
  ...
  function readSynchsafeInteger32At(offset, data) {
    var size1 = ID3.getByteAt(data, offset);
    var size2 = ID3.getByteAt(data, offset + 1);
    var size3 = ID3.getByteAt(data, offset + 2);
    var size4 = ID3.getByteAt(data, offset + 3);
    // 0x7f = 0b01111111
  ...
  function readFrameFlags(data, offset) {
    var flags = {
      message : {
        tag_alter_preservation : ID3.isBitSetAt(data, offset, 6),
        file_alter_preservation : ID3.isBitSetAt(data, offset, 5),
        read_only : ID3.isBitSetAt(data, offset, 4)
      },
      format : {
        grouping_identity : ID3.isBitSetAt(data, offset + 1, 7),
        compression : ID3.isBitSetAt(data, offset + 1, 3),
        encription : ID3.isBitSetAt(data, offset + 1, 2),
        unsynchronisation : ID3.isBitSetAt(data, offset + 1, 1),
        data_length_indicator : ID3.isBitSetAt(data, offset + 1, 0)
      }
    };
  ...
  function readFrames(offset, end, data, id3header, tags) {
    ...
    while (offset < end) {
      ...
      switch( major ) {
        case 2:
          var frameID = frameData.substr(frameDataOffset, 3);
          var frameSize = frameData.getInteger24At(frameDataOffset + 3, true);
          var frameHeaderSize = 6;
          break;

        case 3:
          var frameID = frameData.substr(frameDataOffset, 4);
          var frameSize = ID3.getLongAt(frameData, frameDataOffset + 4, true);
  ...
  ID3v2.readTagsFromData = function(data, tags) {
    var offset = 0;
    var major = ID3.getByteAt(data, offset + 3);
    if (major > 4) {
      return {
        version : '>2.4'
      };
    }
    var revision = ID3.getByteAt(data, offset + 4);
    var unsynch = ID3.isBitSetAt(data, offset + 5, 7);
    var xheader = ID3.isBitSetAt(data, offset + 5, 6);
    var xindicator = ID3.isBitSetAt(data, offset + 5, 5);
    var size = readSynchsafeInteger32At(offset + 6, data);
    offset += 10;

    if (xheader) {
      var xheadersize = ID3.getLongAt(data, offset, true);
